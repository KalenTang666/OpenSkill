...
os init
os discover        # Scan local platforms
os match "react"   # Smart skill matching
...
- `os init` — Initialize wallet
- `os discover` — Auto-scan 8 platforms for configs
- `os import --from claude` — Import platform assets
- `os sync --to codex` — Cross-platform sync
- `os analyze` — 5-dimension quality scoring
- `os scan --all` — 14-rule security scan

### Smart Matching
- `os match "build react app" --stack react,ts` — Context-aware skill recommendations
- Progressive disclosure: metadata only at startup, full instructions on demand

### Migration
- `os migrate --from claude --to codex` — One-click cross-platform migration
- `os backup` / `os restore` — Portable .osp identity bundles

### Hooks (Event-Driven)
- `os hooks --add skill:installed --action notify` — 22 event types
- Actions: log, notify, sync, webhook, script

### Hardware Bridge
- `os devices` — Register desktop/mobile/IoT/wearable
- Cross-device skill sync with capability-based filtering

### Growth System
- 6 ranks, 14 XP actions, 10 achievements
- `os profile` / `os achievements` / `os leaderboard`
